...
def perceptron(feature_matrix, labels, T):
    ...
    theta_0 = 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):  #number of observations
            # Your code here
...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    (nobs, nfeatures) = feature_matrix.shape
    theta = np.zeros(nfeatures)
    theta_sum = np.zeros(nfeatures)
    theta_0 = 0.0
    theta_0_sum = 0.0
    for t in range(T):
        for i in get_order(nobs):
            theta, theta_0 = perceptron_single_step_update(
                feature_matrix[i], labels[i], theta, theta_0)
            theta_sum += theta
            theta_0_sum += theta_0
    return (theta_sum / (nobs * T), theta_0_sum / (nobs * T))
    raise NotImplementedError
#pragma: coderesponse end
...
def pegasos_single_step_update(
    ...
    if label*(np.dot(current_theta, feature_vector)+current_theta_0)<=1:
        return (m*current_theta + eta*label*feature_vector, current_theta_0+eta*label)
 
    return (m*current_theta, current_theta_0)
    raise NotImplementedError
#pragma: coderesponse end
...
def classifier_accuracy(
    ...
    validation_accuracy = accuracy(val_pred, val_labels)
    ...
    for text in texts:
        ...
        for word in word_list:
            if stopwords and word in stopwords:
                continue #this means we skip stopwords and goes to next word
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                if word not in dictionary.keys(): #modified 
